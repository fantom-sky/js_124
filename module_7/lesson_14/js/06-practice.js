...
function handleSubmit(event) {
  ...
  const inputValue = elements.query.value;
  const optionValue = elements.options.value;
  
  // const result = cars.filter((car) => car[optionValue].toLowerCase() === inputValue.trim().toLowerCase())
  const result = cars.filter((car) => car[optionValue].toLowerCase().includes(inputValue.trim().toLowerCase()))
